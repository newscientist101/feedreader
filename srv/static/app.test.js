...
describe('disableNonQueueUI', () => {
  ...
  it('shows overlay on non-queue page', () => {
    setupSidebar();
    // Need a .content element for the overlay
...
describe('enableAllUI', () => {
  ...
  it('does nothing if no offline-disabled elements or overlay exist', () => {
    // Should not throw
    expect(() => window.enableAllUI()).not.toThrow();
  });
});
...
describe('replayPendingActions', () => {
  it('calls callback via setTimeout when no serviceWorker', () => {
    ...
    window.replayPendingActions(cb);
  ...
  it('does not throw when called without callback and no SW', () => {
    delete navigator.serviceWorker;
    expect(() => window.replayPendingActions()).not.toThrow();
  });
  ...
  it('sends GET_PENDING_ACTIONS to SW controller when present', () => {
    ...
    window.replayPendingActions(vi.fn());
  ...
  it('replays dequeue actions and calls callback when SW responds', async () => {
    const postMessage = vi.fn();
    let messageHandler;
    const addEventListener = vi.fn((event, handler) => { messageHandler = handler; });
    const removeEventListener = vi.fn();
    Object.defineProperty(navigator, 'serviceWorker', {
      value: {
        controller: { postMessage },
    ...
    // Mock fetch for both the dequeue api calls and the updateCounts call
    window.fetch = vi.fn().mockResolvedValue({
      ok: true,
    ...
    // Flush microtasks (promise chains) without advancing the 3s safety timeout
    await vi.advanceTimersByTimeAsync(0);

    expect(window.fetch).toHaveBeenCalledWith('/api/articles/123/queue', expect.objectContaining({ method: 'DELETE' }));
    expect(window.fetch).toHaveBeenCalledWith('/api/articles/456/queue', expect.objectContaining({ method: 'DELETE' }));
    expect(cb).toHaveBeenCalledOnce();
...
describe('cacheQueueForOffline', () => {
  it('fetches queue articles and posts to SW', async () => {
    ...
    window.cacheQueueForOffline(sw);

    await vi.runAllTimersAsync();

    expect(window.fetch).toHaveBeenCalledWith('/api/queue', expect.objectContaining({ method: 'GET' }));
  ...
  it('does not throw when fetch fails', async () => {
    ...
    // Should not throw, postMessage should not be called
  ...
  it('does not call postMessage when sw is null', async () => {
    ...
    window.cacheQueueForOffline(null);
...
describe('handleOnlineStateChange', () => {
  it('is a no-op when not in standalone mode', () => {
    // _isStandalone is false because matchMedia is not available in jsdom
    expect(window._isStandalone).toBe(false);

    // Should not throw or create any DOM elements
    window.handleOnlineStateChange();
...
describe('initOfflineSupport', () => {
  it('is a no-op when not in standalone mode', () => {
    ...
    // No offline banner or overlay should be created
    expect(document.getElementById('offline-banner')).toBeNull();
  });
});
...
describe('updateQueueCacheIfStandalone', () => {
  it('is a no-op when not in standalone mode', () => {
    ...
    // Should not throw
    window.updateQueueCacheIfStandalone();
  });
});
